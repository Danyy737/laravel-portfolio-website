...
<div class="max-w-3xl space-y-6">
    ...
    <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg p-6">
        ...
        <p class="mt-1 text-gray-600 dark:text-gray-300">Update the project details below.</p>
    </div>
    ...
    <form method="POST" action="{{ route('admin.projects.update', $project) }}"
        ...
        <div>
            ...
            <textarea name="description" rows="5" required
                      class="mt-1 w-full rounded-md bg-white dark:bg-gray-950 border border-gray-300 dark:border-gray-700
                             text-gray-900 dark:text-gray-100 px-3 py-2">{{ old('description', $project->description) }}</textarea>
            @error('description') <p class="mt-1 text-sm text-red-500">{{ $message }}</p> @enderror
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Tech (comma separated)</label>
            <input name="tech"
                   value="{{ old('tech', is_array($project->tech) ? implode(', ', $project->tech) : $project->tech) }}"
                   placeholder="C#, WinForms, SQL Server"
                   class="mt-1 w-full rounded-md bg-white dark:bg-gray-950 border border-gray-300 dark:border-gray-700
                          text-gray-900 dark:text-gray-100 px-3 py-2">
        ...
        <div>
            ...
            <input name="link" value="{{ old('link', $project->link) }}" placeholder="https://github.com/..."
                   class="mt-1 w-full rounded-md bg-white dark:bg-gray-950 border border-gray-300 dark:border-gray-700
                          text-gray-900 dark:text-gray-100 px-3 py-2">
